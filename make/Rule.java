...
class Rule {
    ...
    void addCommands(List<String> commands) {
        boolean commandsCheck = false;
        if (!commands.isEmpty()) {
            commandsCheck = true;
        }
        if (!commandsCheck || _commands.isEmpty()) {
            for (String s : commands) {
                _commands.add(s);
            }
        } else if (commandsCheck && !_commands.isEmpty()) {
            error("Error: Commands and commandset not empty");
        }
    }
    ...
    private void checkFinishedDependencies() {
        for (int i : _depends.successors(getVertex())) {
            if (_depends.getLabel(i).isUnfinished()) {
                error("Error: Dependencies not built before node is rebuilt.");
            }
        }
    }
    ...
    private boolean outOfDate() {
        if (getTime() == null) {
            ...
            for (int successors : _depends.successors(getVertex())) {
                Integer currentTime = getTime();
                Integer depTime = _depends.getLabel(successors).getTime();
